...
<body>
    <?php include("partials/nav.php"); ?>
        <?php
            if(!isset($_SESSION["tenant_id"]) && !isset($_SESSION["owner_id"]) && !isset($_SESSION["user_owner_id"]) && !isset($_SESSION["user_tenant_id"])){
                ?>
                <form action="http://localhost/PHP_PROJECTS/Rent_House" method="POST">
    ...
    if(isset($_POST["search_button"])){
        if(isset($_POST["search"])){
            ...
            $sql_search_category = "SELECT * FROM apartment WHERE category='$search_category' ORDER BY id DESC";
            $res_search_category = mysqli_query($conn, $sql_search_category);
            if($res_search_category == true){
                $count = mysqli_num_rows($res_search_category);
                if($count > 0){
                    ?>
                    <div class="container">
                        <div class="row">
                    <?php
                    while($row = mysqli_fetch_assoc($res_search_category)){
                    ?>
        ...
        ?>
        <h1>Please Search With Category!!!</h1>
        <?php
    }
    ?>
 ...
 <?php
        ...
        if(isset($_POST["booked_button"])){
            header("location:". "http://localhost/PHP_PROJECTS/Rent_House/search.php");
        }
    ?>